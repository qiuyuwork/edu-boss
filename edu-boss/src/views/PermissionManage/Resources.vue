...
export default {
  ...
  methods: {
    ...
    getResourceCateList() {
        ...
        .then(res=>{
            this.categoryOptions = res.data.content.map(item=>{
                return {label: item.name, value: item.id};
            });
            this.defaultCategoryId=this.categoryOptions[0].value;
        })
    },
    ...
    //删除资源
    handleDelete(row) {
        this.$confirm('能确定删除此资源吗？', '提示', {
            confirmButtonText: '删除',
            cancelButtonText: '取消',
            type: 'warning'
        }).then(() => {
            axios.get("/resource/deleteResource?id="+row.id)
            .then(res=>{
                this.getResourceList();
            }).catch(err=>{
